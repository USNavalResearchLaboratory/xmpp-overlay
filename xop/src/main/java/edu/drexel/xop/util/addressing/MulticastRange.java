 ...
 *         <p/>
 *         TODO: add code to check that start is smaller than end. the other
 *         code assumes that this is the case.
 */
final class MulticastRange {
    private static Logger logger = Logger.getLogger(MulticastRange.class.getName());
    ...
    /**
     * @param range
     *            A multicast range in the form 224.0.0.1-224.0.0.200 (ie: a
     *            dash between the beginning and end of the range)
     * @throws UnknownHostException
     *             There was a problem converting this String into an
     *             InetAddress
     * @throws NotAMulticastAddress
     *             The beginning or end of the range is not a multicast address.
     */
    ...
    public int getSpaceSize() {
        ...
        // convert these numbers into a decimal representation, and then take
        // the difference
        String[] nums = start.split("\\.");
        long start_value = Long.parseLong(nums[0]) * 255 * 255 * 255 + Long.parseLong(nums[1]) * 255 * 255
                + Long.parseLong(nums[2]) * 255 + Long.parseLong(nums[3]);
        nums = end.split("\\.");
        long end_value = Long.parseLong(nums[0]) * 255 * 255 * 255 + Long.parseLong(nums[1]) * 255 * 255
                + Long.parseLong(nums[2]) * 255 + Long.parseLong(nums[3]);

        long size = end_value - start_value + 1;
        if (size > Integer.MAX_VALUE) {
            Logger.getLogger(this.getClass().getName()).severe(
                    "This address space (" + start + "--" + end + ") is way too large!");
        }
    ...
    public InetAddress getAddressByIndex(long address_index) {
        ...
        long start_address = Long.parseLong(nums[0]) * 255 * 255 * 255 + Long.parseLong(nums[1]) * 255 * 255
                + Long.parseLong(nums[2]) * 255 + Long.parseLong(nums[3]);
